...
export default async function CuentaBancariaDocentePage() {
  ...
  return (
    <div className="flex flex-col gap-6 px-4 py-4 md:gap-8 md:py-6 lg:px-8">
      <div className="space-y-3">
        <div>
          <h1 className="text-2xl font-semibold text-slate-900">Cuenta bancaria</h1>
          <p className="text-sm text-muted-foreground">
            Registra los datos de tu cuenta bancaria para recibir pagos.
          </p>
        </div>
      </div>
      ...
      <Card className="border-slate-200 shadow-sm">
        <CardHeader>
          <CardTitle>Datos bancarios del docente</CardTitle>
        </CardHeader>
        <CardContent>
          <FinancialSettingsForm initialData={initialData} showRateFields={false} />
        </CardContent>
      </Card>
    </div>
  )
