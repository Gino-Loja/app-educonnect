...
export async function TeacherReviewsSection({ teacherId, taskId }: TeacherReviewsSectionProps) {
  ...
  const [summaryRes, reviewsRes, myReviewRes] = await Promise.all([
    getTeacherReviewSummary(teacherId),
    listTeacherReviews({ teacherId, limit: 20 }),
    getMyTeacherReview(teacherId),
  ])
  ...
  return (
    <ReviewWidget
      ...
      reviews={reviews}
      taskId={taskId}
      allowReview={currentUser?.role === "student"}
      currentUser={
        currentUser
